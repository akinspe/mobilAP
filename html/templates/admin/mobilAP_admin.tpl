...
<tr class="row<?= $i %2 ? 1 : 2 ?>">
	...
	<td><a href="<?= $App->SCRIPT_NAME ?>?action=edit_session&amp;session_id=<?= $session->session_id ?>"><?= htmlentities($session->session_title) ?></a></td>
...
<?php } else { ?>
<li><?= $session->session_id ?> <a href="<?= $App->SCRIPT_NAME ?>?action=edit_session&amp;session_id=<?= $session->session_id ?>"><?= htmlentities($session->session_title) ?></a></li>
<?php }
	$i++;
} ?>
<?php if ($mobilAP_admin) { ?>
